...
class AgentOrchestrator:
    ...
    async def handle_card_action(
        self, user_id: str, session_id: str, card_id: str, action_id: str,
        payload: Optional[dict], session_manager: SessionManager
    ) -> Dict[str, Any]:
        if action_id == "send":
            ...
            return self._agent_response(session_id, "Action not recognized.", session_manager)

    def _classify_intent(self, content: str) -> str:
        text = content.lower().strip()
        if any(text.startswith(a) for a in ["reply", "approve", "reject", "archive", "forward", "delegate", "snooze"]):
            return "decision_action"
        if any(cmd in text for cmd in ["start stack", "begin stack", "work through emails", "pause", "resume", "next card", "skip"]):
            return "stack_control"
        if any(s in text for s in ["draft", "write", "compose", "prepare a", "create a reply"]):
            return "draft_request"
        if any(s in text for s in ["what did", "did anyone", "show me", "find", "search", "look for", "from", "about", "regarding", "last email", "unread", "recent"]):
            return "kb_query"
        return "general_chat"
    ...
    async def _handle_kb_query(self, user_id, session_id, content, session_manager):
        ...
        if not search_results:
            return self._agent_response(session_id, "I could not find any emails matching that query.", session_manager)
        context = self.kb.summarize_for_agent(search_results)
        lines = [
            "You are an email agent. Be direct and concise.",
            "",
            "The user asked: " + repr(content),
            "",
    ...
    async def _handle_decision_action(self, user_id, session_id, content, session_manager):
        ...
        preview_payload = {
            ...
            "metadata": {"card_id": active_card.id, "draft": draft, "preview": True},
        }
        return self._agent_response(session_id, "Here is the draft. Review before sending.", session_manager, card_payload=preview_payload, cost=draft["cost_usd"])
    ...
    async def _handle_stack_control(self, user_id, session_id, content, session_manager):
        ...
        if any(cmd in text for cmd in ["start", "begin", "let's go", "work through"]):
            ...
            if not card:
                return self._agent_response(session_id, "Your decision stack is empty. No critical emails right now.", session_manager)
            payload = self.stack.to_message_payload(card)
            stack_len = await self.stack.stack_length(user_id)
            return self._agent_response(session_id, "Starting stack. Card 1 of " + str(stack_len) + ":", session_manager, card_payload=payload)
        if "pause" in text:
            return self._agent_response(session_id, "Stack paused. Say resume when you are ready to continue.", session_manager)
        if "next" in text or "skip" in text:
            ...
            card = await self.stack.activate_next(user_id, session_id)
            if not card:
                return self._agent_response(session_id, "Stack complete. No more critical emails.", session_manager, stack_complete=True)
            payload = self.stack.to_message_payload(card)
    ...
    async def _handle_draft_request(self, user_id, session_id, content, session_manager):
        return self._agent_response(session_id, "To draft a reply, either work through your decision stack or reference a specific email by ID.", session_manager)
    ...
    async def _handle_general_chat(self, user_id, session_id, content, session_manager):
        ...
        lines = [
            ...
            "",
            "You have access to the user's inbox. They have not asked a specific question about it right now.",
            "",
            "Recent chat history:",
            history_str,
            "",
    ...
    def _agent_response(self, session_id, text, session_manager, source_email_ids=None, card_payload=None, card_resolved=None, stack_complete=False, cost=None):
        msg = session_manager.add_message(session_id, "agent", text, message_type="card" if card_payload else "text", card_data=card_payload)
        result = {"session_id": session_id, "message": msg, "source_email_ids": source_email_ids or [], "stack_complete": stack_complete}
        if cost is not None:
