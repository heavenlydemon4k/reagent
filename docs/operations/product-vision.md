...
## The Solution
Reagent turns the inbox into a conversation. Bizzy is a persistent teammate that:
...
### 3. Sessions are Real
A decision stack session is a real chat session. It has a title, start time, and history. The user can say "pause" and resume later. Bizzy remembers where they left off.
...
The traditional inbox view exists but is secondary. It's for browsing, searching, and dragging emails into the chat. Bizzy's organizational actions are visible here as labels/badges.
...
2. Bizzy searches KB, finds relevant thread: "Yes, DevOps confirmed the migration is scheduled for Thursday. [Source]"
3. User clicks [Source], sees original email inline.
4. User: "Draft a thank-you reply." Bizzy generates preview. User approves. Send.

### Autonomous Handling
1. Newsletter arrives.
2. Classification: `auto`.
3. Bizzy reads, extracts key info, archives, adds label "Newsletter/Q3".
4. User later asks: "What was in the newsletter today?" Bizzy summarizes from KB.

### Inbox Drag
1. User switches to inbox view.
2. Sees an email from an unknown sender.
3. Drags email into chat.
4. Bizzy analyzes: "This looks like a sales pitch. Want me to draft a polite decline?"
