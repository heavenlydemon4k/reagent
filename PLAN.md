...
| Item | Status | Notes |
|------|--------|-------|
| **Fix `_generate_card` LLM prompt** | ✅ Done (working tree — needs py_compile/tests) | `decision_stack/service.py` `_generate_card` prompt now outputs `{"title","body","question"}`, instructs "no options array — the user responds conversationally," and the JSON-decode fallback emits a `question`. `DecisionCard` dataclass field `options` → `question`. |
| **Fix `to_message_payload`** | ✅ Done (working tree) | `to_message_payload` now emits a `question` string and no `options` key. `_row_to_card` and the `receive_critical_email` `payload_json` persist `question` instead of `options`. |
| **Fix decision-card generation paths** | ✅ Done (working tree) | Decision cards are generated only in `decision_stack/service.py` (orchestrator renders via `to_message_payload`); the live `compression/` path already uses a `need_from_user` question string, so neither decision path emits options. The orchestrator's remaining `options` arrays are the **draft-preview/confirm** cards (Send/Edit/Discard) — the one legitimate button surface per the design log — and are intentionally retained. |
| **Replace prefix-based intent detection** | ✅ Done (working tree) | `orchestrator._parse_action` (brittle `startswith`) replaced by async `_classify_decision_intent`, which prompts the LLM for `{"intent", "params"}` (params: `cc`/`forward_to`/`delegate_to`/`instruction`), validates the intent, and falls back to a keyword heuristic on parse failure. When a card is active, natural-language replies route to the decision handler. Extracted `cc`/`instruction` params are threaded into the draft. |
| **Fix `edit_draft` missing original draft** | ✅ Done (working tree) | `edit_draft` now loads the latest persisted draft via `_load_latest_draft(card_id)`, includes it in the prompt, and persists the revision via `_update_draft`. Required fixing the keying bug: `_persist_draft` stored `card_id=email_id` (FK to `cards.id`); `draft_reply` now takes `card_id` and persists the real card id, so drafts are loadable and `send_and_resolve` resolves. |
| **Profile service persistence** | ✅ Done (working tree) | `profile/service.py` is DB-backed: async `get_or_create` (reads `profiles`, returns defaults without inserting to avoid write-on-read / users-FK risk) and `update` (upsert). All call sites awaited: `orchestrator._handle_general_chat`, `drafting.draft_reply`, and the three `profile/router.py` endpoints. |
...
| Item | Status | Notes |
|------|--------|-------|
| **Document WS architecture decision** | ✅ Done | Recorded in the Design Decision Log ("Sync owns the WebSocket hub"): Sync is the sole client-facing WS endpoint; Intelligence delivers to clients via NATS `sync.broadcast`. |
| **Intelligence → Sync message delivery** | ❌ Unimplemented — **design ready** | Intelligence publishes card/message events on `sync.broadcast` NATS subject with `{user_id, session_id, event}`. Sync consumes and fans out to the user's WS connections via the hub. Full implementation-ready spec (seams, design decisions D1–D5, message contract, file-by-file plan, completion gate) in [`docs/design/sync-broadcast-delivery.md`](docs/design/sync-broadcast-delivery.md). *(Backend; pairs with the decision-state-ownership move.)* |
| **Fix `useWebSocket.ts`** | ✅ Done (working tree — needs `tsc`/`go build`) | Three real mismatches fixed: (1) dev fallback `ws://localhost:8000/chat/ws` → `ws://localhost:8082/ws` (Sync), honoring `VITE_WS_URL`; (2) session was appended as a **path segment** (`/ws/{id}`) which can't match Sync's exact `/ws` route — now `token`/`session_id` are query params; (3) Sync requires `X-Device-ID`, which a browser `WebSocket` cannot set as a header — the client now sends a persisted `device_id` query param, and `sync/internal/websocket/handler.go` `ServeWS` falls back to `?device_id=` when the header is absent. |
| **Deprecate Intelligence WS endpoint** | ⚠️ Redundant | The web client no longer targets `/chat/ws`. Mark it deprecated (internal-only) once the `sync.broadcast` path is built; do not remove until that path is verified. |

**Completion gate:** Client connects to Sync (:8082) `/ws` with `token` + `device_id` query params (no `X-Device-ID` header). `tsc --noEmit` clean; `sync` `go build ./...` clean. A card generated by Intelligence is delivered to the client via the Sync hub. Two clients connected as the same user both receive the card. *(Last two require the `sync.broadcast` delivery path above.)*
