...
# ECS Module
# ------------------------------------------------------------------------------
# FIXME: This production environment only configures 3 of 8 Decision Stack
# services (api, intelligence, voice). The following services are MISSING:
#   - ingestion      (Go — email webhook ingestion)
#   - classification (Go — email classification pipeline)
#   - sync           (Go — card sync, auth, mobile API)
#   - ocr            (Python — document optical character recognition)
#   - stt            (Python — speech-to-text transcription)
#   - tts            (Python — text-to-speech generation)
#   - calendar       (Python — calendar integration service)
#
# TODO: Add all 8 services to the production ECS module call, matching the
# staging environment's service coverage (desired_count_* per service).
# ------------------------------------------------------------------------------
