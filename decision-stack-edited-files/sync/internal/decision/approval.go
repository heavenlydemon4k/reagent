...
// NoOpNatsPublisher is a placeholder NatsPublisher that logs but does not send.
// Use it when NATS is unavailable to keep the service running.
type NoOpNatsPublisher struct{}

// Publish implements NatsPublisher as a no-op.
func (n *NoOpNatsPublisher) Publish(subject string, data []byte) error {
	return fmt.Errorf("NATS publisher not configured")
}
