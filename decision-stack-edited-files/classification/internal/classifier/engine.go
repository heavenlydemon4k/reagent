...
// SetExtractor wires the Extract-Only pipeline after construction.
// Used when the extractor has a dependency on the engine's publisher.
func (e *Engine) SetExtractor(ex Extractor) {
	e.extractor = ex
}
...
func (e *Engine) Classify(ctx context.Context, event models.EmailIngestedEvent) (*models.ClassificationResult, error) {
	...
	extracted := e.tryExtract(ctx, event)
	if extracted != nil {
...
// tryExtract runs the Extract-Only pipeline via the extractor dependency.
func (e *Engine) tryExtract(ctx context.Context, event models.EmailIngestedEvent) *models.ExtractedDatum {
	if e.extractor == nil {
		return nil
	}
	result, err := e.extractor.Process(ctx, &event)
	if err != nil {
		e.log.Debug("extract-only pipeline error", "error", err, "raw_email_id", event.RawEmailID)
		return nil
	}
	if result != nil && result.Route == models.RouteExtract && result.ExtractedData != nil {
		return result.ExtractedData
	}
	return nil
}

// Extractor is the interface for the Extract-Only pipeline.
type Extractor interface {
	Process(ctx context.Context, email *models.EmailIngestedEvent) (*models.ClassificationResult, error)
}
